...
2. **Transitions** — dithered grass↔path edge overlays from `TransitionTiles.ts`
3. **AutoTile** — dual-grid grass↔water shoreline from `AutoTileset.ts` (overrides ground for grass/water cells)
4. **Entities** — objects, NPCs, player (depth-sorted by Y position via `zIndex`)
...
1. **Ground layer** — one sprite per cell, texture from `TileType` (`grass`, `path`, `water`, `wall`, `floor`, `bridge`). This is the fallback look.
2. **Transition layer** — `buildTransitionLayer(map, includeWater=false)` walks every grass cell and adds dithered edge sprites where it borders a `path`/`bridge` tile. Water borders are skipped here because the auto-tileset handles them.
3. **Auto-tile layer** — `buildAutoTileLayer()` from `src/renderer/AutoTileset.ts` is a **dual-grid** wang renderer for grass↔water:
   - Source art is `public/assets/tileset-1.png` — a 4×4 sheet of 16×16 tiles, each encoding which of its 4 corners is grass (vs water).
   ...
   - For each render slot, sample the 4 surrounding cells, build a 4-bit corner mask (`NW=8, NE=4, SW=2, SE=1`, grass=1), look up the tile via `MASK_TO_INDEX`, and draw it scaled to `tileSize`.
   - Anything that isn't water counts as the "upper" terrain (so paths/floors next to water still get a shoreline). Render slots with no grass/water neighbors are skipped so path/floor/wall ground tiles show through.
   - Out-of-bounds cells **clamp** to the nearest in-bounds cell so map borders look natural.
   - **Important:** if you swap `tileset-1.png`, the `MASK_TO_INDEX` table in `AutoTileset.ts` must be regenerated by sampling the new sheet's corners.
...
```typescript
{
  ...
  tileSize: number;               // 32 px (or 16 px — selectable per-map; engine reads this dynamically)
...
The editor reuses `buildTransitionLayer` and `buildAutoTileLayer` from the game renderer, so painted grass↔water tiles render with the same dual-grid shoreline as in-game. The top bar has a 16/32 px tile-size dropdown — `tileSize` is per-map, so different maps can use different pixel densities.
